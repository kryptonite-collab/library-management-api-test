...
唯一 skipped 原因：后端已提供带馆员鉴权的图书写接口；当前自动化尚未接入馆员登录、创建、详情校验、数据库校验和清理闭环，因此该流程暂时 skip。
...
当前测试开发扩展主要关注后端接口质量验证。后端已提供带馆员鉴权的图书写接口；当前自动化尚未接入馆员登录、创建、详情校验、数据库校验和清理闭环，因此该流程暂时 skip。
